...
def run_search(NAME,x_train, y_train, x_val, y_val, num_of_runs = 4):
    ...
    for i in range(num_of_runs):
        ...
        tensorboard = TensorBoard(log_dir='random_search/{}'.format(NAME), update_freq='batch' )
        tuner.search(x = x_train, y = y_train, epochs=15, validation_data=(x_val, y_val), callbacks = [tensorboard])
        best_hps = tuner.get_best_hyperparameters(num_trials=1)[0]
        end = time.time()
        ...
        f.save_evolution_results(number_of_models='', conv1=best_hps.get('conv1'),
                                 ...
                                 kernel3=best_hps.get('kernel_size3'), opt=best_hps.get('optimizer'),
                                 dropout1=best_hps.get('drop1'), dropout2=best_hps.get('drop2'), val_acc='',
                                 number=92, fold_numb=fold_numb, time=elapsed_time/3600, file_name='random_results.csv')
